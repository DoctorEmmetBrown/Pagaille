...
class Frame(qt.QWidget):
    ...
    def _buttonStartRegistration(self):
       ...
       #           'Interpolator': 'BSpline', 'Grid': [GridX, GridY, GridZ]}
        ...
        dicPar = {'Inputs': {'MIM': 0, 'IFIT': IndexIFIT, 'MI': IndexMI, 'IMIT': IndexIMIT, 'InitT': u'Moments',
                             'FI': IndexFI, 'FIM': 0}, \
                  'Optimizer': {'Par': [7.0, 300, 1e-07, 20, 0.0, 7.0, 0.01, 7.0, 2.0, 7.0],
                                'ScalePar': [5.0, 0.01], \
                                'Method': 'Conjugate Gradient Line Search', 'MethodScaling': 'Index Shift'}, \

                  'Outputs': {'Save': [], 'Display': [], 'Sampling': 2.0}, \
                  'Metric': {'GradM': 1, 'Par': [], 'GradF': 1, 'Method': 'Means Squares', \
                             'Sampling': {'Percentage': 0.5, 'Method': u'Random'}}, 'Scaling': [32, 16, 8, 4, 0, 0, 0, 0], \
                  'Interpolator': 'BSpline', 'Grid': [80.0,80.0, 80.0]}
    ...
    def endRegister2(self):
        ...
        Trachea += self.Data_list[self.Seg4Index]
        Trachea[Trachea == 2] = 1.0
        #self.imageSelection.setCurrentRow(self.Seg1Index)
        #self.removeImage()
        #self.imageSelection.setCurrentRow(self.Seg2Index - 1)
        #self.removeImage()
        ...
        self.Vol_normal = (np.sum(np.sum(IC == 1)) - 1) / 1000000.0
        self.Vol_recrut = (np.sum(np.sum(IC == 2)) - 1) / 1000000.0
        self.Vol_r_emphy = (np.sum(np.sum(IC == 3)) - 1) / 1000000.0
        self.Vol_emphysema = (np.sum(np.sum(IC == 5)) - 1) / 1000000.0
        self.Vol_d_emphysema = (np.sum(np.sum(IC == 6)) - 1) / 1000000.0
        self.Vol_new_emphysema = (np.sum(np.sum(IC == 7)) - 1) / 1000000.0
        self.Vol_remain_oedema = (np.sum(np.sum(IC == 8)) - 1) / 1000000.0
        self.Vol_derecrut = (np.sum(np.sum(IC == 9)) - 1) / 1000000.0
        ...
        self.text += '--------------------------------------------------------' + '\n'
        self.text += 'Clusters : ' + '\n'
        self.text += 'Normal [L] : ' + str(self.Vol_normal) + '\n'
        self.text += 'Recruitment Oedema to Normal[L] : ' + str(self.Vol_recrut) + '\n'
        self.text += 'Recruitment Oedema to Emphysema[L] : ' + str(self.Vol_r_emphy) + '\n'
        self.text += 'Permanent Emphysema [L] : ' + str(self.Vol_emphysema) + '\n'
        self.text += 'Derecrutment Oedema to Emphysema [L] : ' + str(self.Vol_d_emphysema) + '\n'
        self.text += 'Normal To Emphysema : ' + str(self.Vol_new_emphysema) + '\n'
        self.text += 'Permanent Oedema : ' + str(self.Vol_remain_oedema) + '\n'
        self.text += 'Derecruitment Oedema To normal : ' + str(self.Vol_derecrut) + '\n'
