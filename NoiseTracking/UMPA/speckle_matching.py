...
if __name__ == "__main__":
    ...
    nbMeasurements,w,h= measurements.shape
    for i in range(nbMeasurements):
        array=measurements[i,:,:]
        arrayNormalized=65535*(array-array.min())/(array.max()-array.min())
        arrayNormalized=arrayNormalized.astype(np.int16)
        texti='%4.4d'%i
    ...
    transmission=65535*(transmission-transmission.min())/(transmission.max()-transmission.min())
    transmission=transmission.astype(np.int16)
    ...
    dy=result['dy']
    dy=255*(dy-dy.min())/(dy.max()-dy.min())
    dy=dy.astype(dy.int8)
    imsave('dyReference.tif',dy)

    df=result['df']
    df=255*(df-df.min())/(df.max()-df.min())
    df=df.astype(df.int8)

    imsave('darkfieldReference.tif',df)
    f=result['f']
    f=255*(f-f.min())/(f.max()-f.min())
    f=f.astype(f.int8)

    imsave('functionReference.tif',f)
